...
import { getRecentTrips, type Trip, type TripStatus } from '@/lib/mock-data';
...
export default function TripsPage() {
  ...
  const handleUpdateTrip = async (updatedTrip: Trip) => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: 'El viaje ha sido actualizado.',
      });
    } else {
        toast({
            variant: 'destructive',
  ...
  const activeServices = trips.filter((trip) =>
    ['Asignado', 'En Progreso', 'Programado'].includes(trip.status)
  );
  ...
  return (
    <>
      ...
      />
      {isLoading ? (
        <div className="text-center p-8">Cargando viajes...</div>
      ) : (
